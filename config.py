...
# === 开仓后计划委托: 止盈止损 (2026-09-01 娜姐从 ±90% 调整为 299%止盈 / 多单止损99%空单止损299%) ===
# 注意: 全仓5x强平线约-80%, 此超宽SL远超强平线, 实际不会提前触发(仅止盈+移动止盈生效)
SL_LONG_PCT = 0.99    # 多单止损: 开仓价×0.01 (-99%)
SL_SHORT_PCT = 2.99   # 空单止损: 开仓价×3.99 (+299%)
TP_LONG_PCT = 2.99    # 多单止盈: 开仓价×3.99 (+299%)
TP_SHORT_PCT = 0.99   # 空单止盈: 开仓价×0.01 (-99%)

# === 移动止盈（每仓独立, 回撤止损）===
# 多单涨幅 / 空单跌幅 达到激活阈值后, 开始跟踪最高/最低价;
# 激活后从最高(多)/最低(空)位回撤超过回撤阈值 → 市价平该仓。
TRAILING_ACTIVATE_PCT = 0.06   # 涨跌幅≥6% 激活移动止盈
TRAILING_DRAWDOWN_PCT = 0.04   # 激活后从极值回撤 ≥4% 平仓
TRAILING_STATE_FILE = os.path.join(os.path.dirname(__file__), "trailing_state.json")  # 状态持久化(重启不丢)

# === 仓位跟踪轮询间隔(秒) ===
# tracker 每次拉持仓 + 最新价, 要求每 TRACKER_POLL_SECONDS 秒刷新一次行情。
TRACKER_POLL_SECONDS = 3
